...
// a function that finds the average of Vx and Vy

float** average(int k,  float*** OptFl_Vxy,int length,int width, float** VxyM_ij) {
    ...
    for ( i = 1; i < length-1; i++)
      {
        for ( j = 1; j < width-1; j++)
        {          
          VxyM_ij[i][j] = (OptFl_Vxy[k-1][i-1][j] + OptFl_Vxy[k-1][i+1][j] + OptFl_Vxy[k-1][i][j+1] + OptFl_Vxy[k-1][i][j-1]) / 6.0 + (OptFl_Vxy[k-1][i-1][j-1] + OptFl_Vxy[k-1][i-1][j+1] + OptFl_Vxy[k-1][i+1][j+1] + OptFl_Vxy[k-1][i+1][j-1])/12.0;
        }
      }
      return VxyM_ij;
...
// the same intensity function applies to Ix, Iy and It

void intensity_x_y_t(float** Img1, float** Img2, int length, int width, float** Ix, float** Iy, float** It){
  
  for (int i = 0; i < length-1; i++)
    {
      for (int j = 0; j < width-1; j++)
      {
        Ix[i][j] = (Img1[i][j+1] - Img1[i][j] + Img1[i+1][j+1] - Img1[i+1][j] + Img2[i][j+1] - Img2[i][j] + Img2[i+1][j+1] - Img2[i+1][j])/4.0;
        Iy[i][j] = (Img1[i+1][j] - Img1[i][j] + Img1[i+1][j+1] - Img1[i][j+1] + Img2[i+1][j] - Img2[i][j] + Img2[i+1][j+1]- Img2[i][j+1]) / 4.0;
        It[i][j] = (Img2[i][j] - Img1[i][j] + Img2[i+1][j] - Img1[i+1][j] + Img2[i][j+1] - Img1[i][j+1] + Img2[i+1][j+1] + Img1[i+1][j+1]) / 4.0;
      }
    }
}
...
float jacobiX(float Ix, float Iy, float It, float alpha, float VxM, float VyM){ //V soit Vx ou Vy

  return VxM - Ix * ((Ix * VxM + Iy * VyM + It) / (alpha * alpha + Ix * Ix + Iy * Iy));

}

float jacobiY(float Ix, float Iy, float It, float alpha, float VxM, float VyM){ //V soit Vx ou Vy

  return VyM - Iy * ((Ix * VxM + Iy * VyM + It) / (alpha * alpha + Ix * Ix + Iy * Iy));
...
int main(int argc,char** argv)
{
 ...
 //
 // L'image 1 et 2 sont enregistrée dans les tableau 2D Img1[][] et Img2[][]   ==> on l a deja des qu on upload l'image <==  donc on a lintensity c est bon
 // Ix, Iy, It sont des tableaux 2D de même dimension que l'image qui ont été allouées
 // VxM, VyM  Vecteur Vx & Vy moyenné aussi   ==> il faut calculer la moyenne des vecteurs de mouvement de cahque pixel <==
...
intensity_x_y_t(Img1, Img2, length, width, Ix, Iy, It);
  
  for ( k = 1; k < NBITER; k++) // remplir le tab 3D de optFl_Vx et optFl_Vy 
  { 
    
    printf("\riteration[%d/%d]", k, NBITER);
    fflush(stdout); // Flush the output buffer to ensure the text appears immediately
    for ( i = 0; i < length; i++)
    {
      for ( j = 0; j < width; j++)
      {

        OptFl_Vx[k][i][j] = jacobiX(Ix[i][j], Iy[i][j], It[i][j], alpha, VxM[i][j], VyM[i][j]); // 
        OptFl_Vy[k][i][j] = jacobiY(Ix[i][j], Iy[i][j], It[i][j], alpha, VxM[i][j], VyM[i][j]); // 

      }
    }
    VxM = average(k, OptFl_Vx, length, width, VxM); // remplir tab2D avec nouvelle valeur de VxM lorsque on incremente k de 1 
    VyM = average(k, OptFl_Vy, length, width, VyM); // remplir tab2D avec nouvelle valeur de VyM lorsque on incremente k de 1
  }
