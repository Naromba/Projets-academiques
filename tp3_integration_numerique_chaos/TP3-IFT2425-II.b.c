...
void trapeze(float* resultats, int nombreIntervalles) {
    // Borne inférieure de l'intervalle
    double borneInf = 0.0;
    // Borne supérieure de l'intervalle
    double borneSup = 1.0;
    // Largeur de chaque intervalle
    double largeurIntervalle = (borneSup - borneInf) / nombreIntervalles;

    // Calcul des valeurs aux points équidistants dans l'intervalle [0, 1]
    for(int i = 0; i <= nombreIntervalles; i++) {
        // Point dans l'intervalle
        double x = borneInf + i * largeurIntervalle;
        // Évaluation de la fonction f(x) à ce point et stockage dans le tableau des résultats
        resultats[i] = largeurIntervalle * fonction(x);
    }
}

// Sommation par paires récursive
double somme_par_paires(float* vecteur, int debut, int fin) {
    if (debut == fin) { // Cas de base : une seule valeur à additionner
        return vecteur[debut];
    } else if (debut + 1 == fin) { // Cas de base : deux valeurs à additionner
        return vecteur[debut] + vecteur[fin];
    } else {
        // Division en deux moitiés et récursion
        int milieu = (debut + fin) / 2;
        double somme_gauche = somme_par_paires(vecteur, debut, milieu);
        double somme_droite = somme_par_paires(vecteur, milieu + 1, fin);
        return somme_gauche + somme_droite;
    }
}

double somme_kahan (float* vecteur, int n){
    double somme = 0.0; //somme cumulee
    double erreur  = 0.0; // erreur d'arrondi

    for (int i = 0; i < n; i++)
    {
        double temps = somme;
        double y = vecteur[i] + erreur;
        somme  = temp +y;
        erreur = (temp - somme) + y ;
    }
...
//--------------------------------------------------------------------------------
 ...
 //Programmer ici
 // Création du vecteur de flottants avec N = 5 000 001 points
int N = 5000001;
float* vecteur = new float[N];

// Remplissage du vecteur avec les valeurs flottantes de la méthode des trapèzes
trapeze(vecteur, N);

// Appel de la sommation par paires récursive
double somme_paires = somme_par_paires(vecteur, 0, N - 1);

// Affichage du résultat avec une précision de 10 chiffres significatifs après la virgule
std::cout << "Estimation de la somme par paires : " << std::setprecision(10) << somme_paires << std::endl;

// Appel de la somme compensée de Kahan
double somme_de_kahan = somme_kahan(vecteur, N);
std::cout << "Estimation de la somme compensée de Kahan : " << std::setprecision(10) << somme_de_kahan << std::endl;
