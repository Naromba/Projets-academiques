...
double trapeze(int nombreIntervalles) {
    double borneInf = 0.0; // Borne inférieure de l'intervalle
    double borneSup = 1.0; // Borne supérieure de l'intervalle
    double largeurIntervalle = (borneSup - borneInf) / nombreIntervalles; // Largeur de chaque intervalle
    double somme = fonction(borneInf) + fonction(borneSup); // Somme des valeurs aux bornes

    // Calcul de la somme des valeurs aux points intermédiaires
    for(int i = 1; i < nombreIntervalles; i++) {
        double pointIntermediaire = borneInf + i * largeurIntervalle;
        somme += 2 * fonction(pointIntermediaire);
    }

    // Application de la formule des trapèzes
    return (largeurIntervalle / 2) * somme;
}
...
//--------------------------------------------------------------------------------
 ...
 result = trapeze(NbInt);
 
 // Calcul de l'erreur
 double erreur = fabs(PI - result);
