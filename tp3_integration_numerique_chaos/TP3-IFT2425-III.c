...
void bifurcation (float** Graph2D, int lenght, int width){
    for (double mu = 2.4 ; mu <= 4; mu += 0.0001 )
    {
        float x =0.5;
        for (int i = 0; i < 20000; ++i)
        {
            x = mu * x * (1 - x);
            if (i >= 10000){
                int j = (int)((mu - 2.5) / 0.0001);
                int k = (int) (x * 4096);
                if (j >= 0 && j < 4096 && k >= 0 && k < 4096){
                    Graph2D[j][k] = 0.0;
                }
            }
        }
...
/*
void bifurcation(float** Graph2D, int length, int width) {
    float u_min = 2.5;  // Début de la variation de u
    float u_max = 4.0;
    float pas = 0.0001;
    int n_iterations = 20000;  // Nombre total d'itérations
    int n_skip = 10000;        // Nombre d'itérations à sauter

    for (float mu = u_min; mu <= u_max; mu += pas) {
        float x = 0.5;
        int j = (int)((mu - 2.5) / 0.0001);
...
//--------------------------------------------------------------------------------
 ...
 float** Graph2D=fmatrix_allocate_2d(4096, 4096);
 bifurcation(Graph2D, 4096, 4096);
