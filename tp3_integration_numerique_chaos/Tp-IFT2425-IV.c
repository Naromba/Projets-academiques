...
void bifurcation(float** Graph2D, int width, int length){
    float u_min = 2.5, u_max = 4.0;
    float pas = 0.0001; // Pas de mu
    float x_n = 0.5; // Valeur initiale de x
    int n = 20000; // Nombre total d'itérations

    for(float u = u_min; u < u_max; u += pas){
        int r = (int)((u - u_min) * width / (u_max - u_min));
        for(int j = 0; j < n; j++){
            x_n = u * x_n * (1 - x_n);
            if(j >= n / 2){
                int x = (int)(length * (1 - x_n));
                if(r >= 0 && r < width && x >= 0 && x < length){
                    Graph2D[x][r] = 0.0; // Mettre en noir le point
                }
            }
        }
        x_n = 0.5; // Réinitialiser pour la prochaine itération de mu
    }
}
