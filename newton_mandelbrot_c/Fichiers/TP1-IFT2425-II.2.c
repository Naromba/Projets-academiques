...
// author  : Naromba Condé - Mounir Amman
...
for (int i = 0; i < length; i++){
    for ( int  j = 0; j < width; j++)
    {
        ...
        // a et b représentent les parties réelles et imaginaires du nombre complexe z
        // a2 et b2 represente leurs carrés
        double a = 0.0;
        double b = 0.0;
        double a2 = 0.0;
        double b2 = 0.0;
        int k;
        for (k=0; k<NbIterMax && (a2 + b2 <= 4.0); k++){
            double aTemp = a2 - b2 + x;
            b = 2 * a * b + y ;
            a = aTemp;
            a2 = a * a;
            b2 = b * b;
        ...
        if (k == NbIterMax) {
        //l'intérieur de l'ensemble de Mandelbrot en blanc
            Graph2D[i][j] = 255;
        } else {
        //l'extérieur de l'ensemble de Mandelbrot 
            Graph2D[i][j] = k;
    }
