//------------------------------------------------------
// module  : Tp-IFT2425-I.4.c
// author  : Naromba Conde - Mounir Ammam
// date    : 16 FEVRIER 2024
// version : 1.0
// language: C
// note    :
//------------------------------------------------------
...
//--------------------------------------------------------------------------------
 ...
 for(int i=0;i<length;i++) for(int j=0;j<width;j++) Graph2D[i][j]=j/2.0;
   ...
   // Note : pour cette partie du devoir, j'ai demandé des explications 
   //à chagpt sur comment aborder le sujet.
int NbIterMax = 200; 
double minX = -2.0;
double maxX = 1.0;
double minY = -1.5;
...
for (int j = 0; j < length; j++) {
    for (int i = 0; i < width; i++) {
        // positionnement du pixel à un nombre complexe
        double x = minX + (i * (maxX - minX) / width);
        double y = minY + (j * (maxY - minY) / length);

        double a = 0.0;
        double b = 0.0;

        int k;
        for (k = 0; k < NbIterMax; k++) {
            // Appliquer l'algorithme de Mandelbrot
            double a_temp = a * a - b * b + x;
            double b_temp = 2 * a * b + y;
            
            a = a_temp;
            b = b_temp;

            // Vérifier si le point ne passe à l'infini
            if ((a * a + b * b) > 4.0) {
              break;
            }
        }
